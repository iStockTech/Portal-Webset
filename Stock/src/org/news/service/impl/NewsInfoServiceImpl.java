...
public class NewsInfoServiceImpl implements NewsInfoService {
	...
	public boolean addNewsInfo(NewsInfo newsInfo,
			List<java.io.File> attachment, List<String> attachmentFileName)
			throws IOException {
	...
	public boolean deleteNewsInfo(int[] newsInfoIds) {
		for (int i = 0; i < newsInfoIds.length; ++i) {
			// 有附件才删除附件
			if (attachmentDAO.findNewsAttachmentByNewsId(newsInfoIds[i]).size() > 0
					&& !attachmentDAO
							.deleteNewsAttachmentByNewsId(newsInfoIds[i])) {
				return false;
			}
		}
	...
	public boolean updateNewsInformation(NewsInfo newsInfo,
		...
		if (newsInfoDAO.updateNewsInformation(newsInfo) == null) {
			...
			for (int i = 0; i < attachment.size(); i++) {
				...
				newsAttachment.setAttachmentName(attachmentFileName.get(i));
				newsAttachment
						.setAttachmentContent(readFile(attachment.get(i)));
				if (!attachmentDAO.addNewsAttachment(newsAttachment)) {
					return false;
				}
			}
	 ...
	 * @return 新闻集合
	 */
	public List<NewsInfo> getAllNewsInfo(String keyword, int currentPage,
			int lineSize) {
	...
	public List<NewsIndex> getNewsByTypes(String[] newsTypes) {
		...
		for (int i = 0; i < results.size(); ++i) {
			index.add(this.toNewsIndex(results.get(i)));
		}
		return index;
	}
	 ...
	 * @return 新闻集合
	 */
	public List<NewsIndex> getNewsByTypes(String[] newsTypes, int currentPage,
			int lineSize) {
	...
	public NewsVO toNewsVO(NewsInfo newsInfo) {
		...
		if (newsInfo != null) {
			...
			newsVO.setAdminName(adminDAO.findAdminById(newsInfo.getAdminId())
					.getAdminName());
